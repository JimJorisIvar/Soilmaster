...
//        -----------------------------------------------------------------
    echo '<marker id="'.$scan->getScanid().'" data-scanid="' . $scan->getScanid() . '" date="Scan van:  '.$scan->getDate().'" koper="koper: '.$scan->getKoperPpm().' PPM" address="ADDRESS HIER" name="'.$scan->getScanName().'" lat="'.$lat.'" lng="'.$lng.'" type="' . $months .'"/>';};?>
</markers>
